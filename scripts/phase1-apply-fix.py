#!/usr/bin/env python3
"""Phase 1: Apply fix on VPS, commit, push to GitHub, rebuild platform."""
import sys
...
def main():
    ...
    print('\n=== Step 4: Rebuild lipehost platform (this takes ~3 min) ===')
    run(ssh, f"cd {PROJECT_DIR} && NODE_ENV=production bun run build 2>&1 | tail -25", timeout=600)

    # 5. Restart lipehost service
    print('\n=== Step 5: Restart lipehost service ===')
    run(ssh, 'systemctl restart lipehost && sleep 3 && systemctl status lipehost --no-pager | head -8', timeout=60)

    # 6. Verify platform up
    print('\n=== Step 6: Verify platform up ===')
    run(ssh, 'curl -sS -o /dev/null -w "LipeHost: HTTP %{http_code}\\n" http://localhost:3000/', timeout=30)

    print('\n🎉 PHASE 1 DONE — fix pushed and platform rebuilt!')
    ssh.close()
