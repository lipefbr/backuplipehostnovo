...
import {
  ...
  Check, ShieldCheck, Server, Github, Chrome, Apple,
} from 'lucide-react'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'register') {
      ...
      try {
        ...
        // Auto-login after register — same redirect:true approach
        setSuccess(true)
    ...
    // Login mode — use redirect:true (native form POST, no fetch needed)
    setLoading(true)
    setSuccess(true)
    // signIn with redirect:true does a full page navigation (form POST)
    // No fetch() is used — the browser submits the form natively
    // NextAuth sets the cookie in the HTTP response, then redirects to /painel
    signIn('credentials', {
      email: form.email,
      password: form.password,
      redirect: true,
  ...
  const renderLoginCardInner = () => (
    <React.Fragment>
      ...
      <div className="flex justify-center mb-6">
        <Link href="/" className="flex items-center" aria-label="LipeHost">
          <Image
      ...
      <div className="mb-7 text-center">
        ...
        <p className="text-sm text-white/55">
          {mode === 'login'
            ? 'Acesse seu painel para gerenciar projetos e sistemas'
        ...
        {success && (
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs text-emerald-300/70">Redirecionando...</div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0, height: 'auto' }}
            exit={{ opacity: 0, height: 0 }}
            className="mb-5 rounded-xl border border-red-500/30 bg-red-500/10 p-3.5 flex items-center gap-3"
          >
            <div className="size-7 rounded-full bg-red-500/20 flex items-center justify-center flex-shrink-0">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {mode === 'register' && (
          <div>
            <Label htmlFor="name" className="text-xs text-white/70 mb-1.5 block">Nome completo</Label>
        ...
        <div>
          <Label htmlFor="email" className="text-xs text-white/70 mb-1.5 block">E-mail</Label>
          <div className="relative">
        ...
        {mode === 'register' && (
          <div>
            <Label htmlFor="confirm" className="text-xs text-white/70 mb-1.5 block">Confirmar senha</Label>
            <div className="relative">
              <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 size-4 text-white/40" />
              <Input
                id="confirm"
      ...
      <div className="grid grid-cols-3 gap-2">
        <button type="button" className="h-10 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.08] flex items-center justify-center text-white/70 hover:text-white transition-all" aria-label="Entrar com Google">
          <Chrome className="size-4" />
        </button>
        <button type="button" className="h-10 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.08] flex items-center justify-center text-white/70 hover:text-white transition-all" aria-label="Entrar com GitHub">
          <Github className="size-4" />
        </button>
        <button type="button" className="h-10 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.08] flex items-center justify-center text-white/70 hover:text-white transition-all" aria-label="Entrar com Apple">
      ...
      <p className="text-center text-xs text-white/50 mt-6">
        {mode === 'login' ? (
          <>
            Não tem conta?{' '}
            <button type="button" onClick={() => setMode('register')} className="text-blue-400 hover:text-blue-300 font-semibold transition-colors">
              Registrar-se
            </button>
          </>
        ) : (
          <>
            Já tem conta?{' '}
            <button type="button" onClick={() => setMode('login')} className="text-blue-400 hover:text-blue-300 font-semibold transition-colors">
              Entrar
            </button>
          </>
        )}
      ...
      <main className="flex-1 pt-32 pb-20 relative overflow-hidden">
        ...
        <div className="container-x relative">
          <div className="max-w-md mx-auto">
            {isMobile ? (
              <div className="relative rounded-3xl border border-white/10 bg-[#0c0c0c]/80 backdrop-blur-xl p-8 sm:p-10 overflow-hidden">
                {renderLoginCardInner()}
              </div>
            ) : (
              <motion.div
